...
CRUMB_RESPONSE=$(curl -s -c /tmp/jenkins_cookies.txt -u "$USER:$PASS" "$JENKINS_URL/crumbIssuer/api/xml?xpath=concat(//crumbRequestField,\":\",//crumb)" || true)
echo "CRUMB_RESPONSE: '$CRUMB_RESPONSE'"

CRUMB_HEADER_ARGS=()
COOKIE_ARGS=()
if [[ -n "$CRUMB_RESPONSE" ]]; then
  IFS=":" read -r CRUMB_FIELD CRUMB_VALUE <<< "$CRUMB_RESPONSE" || true
  echo "Using crumb header: $CRUMB_FIELD"
  CRUMB_HEADER_ARGS=(-H "$CRUMB_FIELD: $CRUMB_VALUE")
  COOKIE_ARGS=(-b /tmp/jenkins_cookies.txt)
else
  echo "No crumb (CSRF) info - continuing without crumb header"
fi

# check if job exists (HTTP 200)
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "${COOKIE_ARGS[@]:-}" -u "$USER:$PASS" "$JENKINS_URL/job/$JOB_NAME/api/json" || true)
if [[ "$HTTP_CODE" == "200" ]]; then
  echo "Job already exists (HTTP $HTTP_CODE) — updating its config.xml..."
  curl -v -u "$USER:$PASS" "${COOKIE_ARGS[@]:-}" "${CRUMB_HEADER_ARGS[@]:-}" -X POST \
...
curl -s "${COOKIE_ARGS[@]:-}" -u "$USER:$PASS" "$JENKINS_URL/job/$JOB_NAME/api/json" -o /tmp/jenkins_job.json || true
...
curl -s -u "$USER:$PASS" "${COOKIE_ARGS[@]:-}" "${CRUMB_HEADER_ARGS[@]:-}" -X POST "$JENKINS_URL/job/$JOB_NAME/build" -o /tmp/jenkins_trigger_body.txt || true
...
for i in {1..60}; do
  sleep 1
  JSON=$(curl -s "${COOKIE_ARGS[@]:-}" -u "$USER:$PASS" "$JENKINS_URL/job/$JOB_NAME/api/json" || true)
...
sleep 2
curl -s "${COOKIE_ARGS[@]:-}" -u "$USER:$PASS" "$JENKINS_URL/job/$JOB_NAME/$BUILD_NUM/consoleText" || true
